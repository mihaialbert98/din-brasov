...
export async function PATCH(
  ...
  const { date, time, partySize, floorTableIds, force } = parsed.data;

  const result = await updateReservation(restaurant.id, reservationId, { date, time, partySize, floorTableIds }, !!force);
  if (!result.ok) {
    return NextResponse.json({ error: result.reason, overridable: result.overridable }, { status: 400 });
  }
  return NextResponse.json({
    ok: true,
    notifiableByEmail: result.notifiableByEmail,
    // Non-empty only when the move made a table staff had chosen unusable.
    floorTablesCleared: result.floorTablesCleared,
  });
}

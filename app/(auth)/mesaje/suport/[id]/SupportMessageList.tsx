...
export default function SupportMessageList({
  ...
  initialMessages: Msg[];
  // When viewing as admin: the user who owns the conversation
  convUserId?: string;
  convUserName?: string;
}) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const res = await fetch(`/api/support/conversations/${conversationId}/messages`, { cache: "no-store" });
      if (!res.ok) return;
  ...
  useEffect(() => {
    (window as any).__refreshSupportMessages = fetchMessages;
    return () => { delete (window as any).__refreshSupportMessages; };
  }, [fetchMessages]);

 ...
 * (AdminClientsManager) for both "email all subscribers" and "email selected".
 */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-lg w-full p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-xs text-gray-500 mb-4">
          {scoped
            ? "Se trimite doar clienților selectați care s-au abonat la newsletter (consimțământ). Fiecare email are link de dezabonare."
            : "Se trimite tuturor clienților care s-au abonat la newsletter (consimțământ). Fiecare email are link de dezabonare."}
        </p>

        {result ? (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-sm text-green-800">
            Trimis: {result.sent} · Sărite: {result.skipped} · Eșuate: {result.failed}
            <button onClick={onClose} className="block mt-3 text-xs font-semibold underline">Închide</button>
          </div>
        ) : (

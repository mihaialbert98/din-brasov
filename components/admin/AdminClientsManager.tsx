...
export interface AdminClient {
  key: string; // lower(email) — stable selection key (account or guest)
  name: string | null;
  email: string;
  phone: string | null;
  visits: number;
  ...
  isGuest: boolean; // account-holder vs email-only guest
}

/**
 * Admin per-restaurant clients list with selection. Covers account-holders AND
 * guests who left an email. Only CONSENTED clients (active newsletter subscribers)
 * are selectable/emailable; the rest are shown for context but can't be picked.
 * "Email selectați" sends to the current-page selection; "Email toți abonații"
 * targets every subscribed client. Selection is by email, scoped to the current page.
 */
export default function AdminClientsManager({
  ...
  const allEmailableSelected =
    emailableOnPage.length > 0 && emailableOnPage.every((c) => selected.has(c.key));
  ...
  function toggleAll() {
    setSelected((prev) => {
      const n = new Set(prev);
      if (emailableOnPage.every((c) => n.has(c.key))) {
        emailableOnPage.forEach((c) => n.delete(c.key));
      } else {
        emailableOnPage.forEach((c) => n.add(c.key));
      }
      return n;
    });
  }
  ...
  return (
    <div className="space-y-4">
          ...
          <div key={c.key} className="p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {c.isGuest && (
                  <span className="text-[11px] bg-blue-50 text-blue-700 rounded-full px-2 py-0.5" title="A rezervat ca invitat (fără cont).">
                    invitat
                  </span>
                )}
                {c.subscribed ? (
      ...
      {selectedEmails.length > 0 && (
        <div className="sticky bottom-4 z-10 flex items-center justify-between gap-3 bg-white border border-gray-200 shadow-lg rounded-xl px-4 py-3">
          <span className="text-sm text-gray-700">
            {selectedEmails.length} {selectedEmails.length === 1 ? "selectat" : "selectați"}
          </span>
      ...
      {composer === "selected" && (
        <EmailClientsComposer restaurantId={restaurantId} restaurantName={restaurantName} emails={selectedEmails} onClose={() => setComposer(null)} />
      )}
    </div>
  );
}
